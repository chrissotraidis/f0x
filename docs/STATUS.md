...
| Touch controls | Physical iPad and iPhone layouts accepted | The F0X touch system writes direct atomic N64 pad state at the port-1 seam and passes all 87 focused sub-checks. The latest owner-accepted iPad and iPhone profiles are independent source defaults, including per-control scales and the phone's hidden D-pad. The phone ••• is fixed to the upper-right safe area and reasserted above transient UIKit reordering. Stress multi-touch, haptics, controller handoff, interruptions, and broader phone gameplay remain open. |
...
| App identity / public README | Physical-iPad hero approved; public IPA still pending | Original opaque 1024x1024 F0X artwork remains the compiled Apple app icon. The README now uses an owner-approved physical-iPad racing image as its hero and records iPad acceptance separately from the newly installed, not-yet-accepted iPhone build. No public IPA, App Store, or TestFlight build is claimed. |
...
5. Publish a public IPA only after the phone acceptance and package audit pass.
