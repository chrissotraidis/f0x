...
| Lifecycle / 60 Hz / high refresh | 60 Hz macOS measured; iOS SDL audio and guarded race stability verified; mobile lifecycle partial; high refresh not started | macOS simulation/timer measured at 59.954 race frames/s over a 37.4 s scripted Mute City race, with race-window p50 16.6 ms / p99 17.9-19.2 ms / 0 spikes and audio-thread p95 2.9-4.0 ms. On iOS, libultraship now defaults to SDL exactly as the pinned HarkinianPad patch does; a rebuilt iPhone 17 Pro Simulator launch logged `SDL Audio initialized: 2 channels, 32000 Hz`, active dedicated producer, and live BGM synthesis. IOS-GFX-CRASH-01's independent converted-dialect side map was removed in favor of cache-owned metadata; rebuild/eviction regression passes, and a guarded 5:16 scripted Simulator race/transition soak produced no crash report. Simulator Home/relaunch restores touch neutrally. Representative courses, physical audible route, full lifecycle suspension, high refresh, and Low Power Mode remain open. |
