...
| 3: complete macOS Metal race | Partially working | Native Metal process boots a verified raw ROM, starts the game fibers/audio thread, accepts deterministic Start input, and reaches a real display-list task. Captures before and after Start are all black; title/race are not verified. |
