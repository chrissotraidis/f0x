...
| 4: macOS application | Partially working | A clean, ad-hoc-signed `F0X.app` Debug bundle passes strict macOS signature validation, carries correct F0X metadata, keeps mutable state in `~/Library/Application Support/F0X`, and directly launches its branded Metal cartridge-ROM setup window. Completed import, saves, Developer ID signing/notarization, and distribution remain unproven. |
